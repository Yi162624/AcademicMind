...
| 模式 | 用户输入 | 系统输出 |
| :--- | :--- | :--- |
| **领域调研** | 一个研究问题（如"大模型在医疗影像的应用"） | ① Markdown 调研报告（纯文字，图片素材搜索规划中） ② 3 个值得深入研究的方向建议 ③ 5 篇最值得精读的核心论文清单 |
| **论文分析** | 1 篇或多篇论文（链接或 PDF） | 单篇：深度精读报告（带出处锚点与批判性分析）；多篇：① 综述分析报告（方法/实验/结果/局限/结论） ② 论文间对比与关联 ③ 推荐精读论文 + 下一步行动建议 |
...
| 报告形态 | Markdown 文字报告 | 单篇：深度精读报告（skill）；多篇：对比综述报告（5 Agent） |
...
**第 3 步：写作者整合生成**
- 综合所有证据，生成 Markdown 文字报告
- 正文引用来源处标注来源论文标题（供 Verifier 核对引用真假）
- 配图预留：图片搜集本期未接入（见上文"图片素材搜集暂未接入"），报告当前为纯文字
...
用户问题 → Planner → ResearchTask[]（研究任务单：要回答什么/为什么/要什么证据/服务哪章）
        ...
        → Writer → Report（正文引用出处标注「来源：论文标题链接」，不暴露内部编号）
        → Verifier → 沿 Evidence → Claim → Source 逐条核对（论断是否有出处、来源是否真实）
...
- Writer 正文只在引用论文处标注「来源：论文标题链接」（可点开核对的出处），不向读者暴露内部编号
- 这一契约让 Verifier 从"报告 vs 摘要模糊比对"升级为"沿证据链核对"，Advisor 从"只吃报告正文"升级为"基于研究任务和证据概况判断研究空白"
...
```
论文输入 → 解析分流 → PDF 解析 → 图表理解 → 推理知识库 → 一次性生成精读报告
```

**解析阶段（确定性工程，代码完成）**：
- 输入分流：本地上传 PDF 直接使用；比赛合规模式（RESEARCH_SOURCE=domestic）下仅允许本地上传，不走联网下载
- PDF 解析：Docling 统一解析（文本 + 表格 + 图片页）
- 图表理解：Docling 提取的图表通过 Qwen3-VL-Plus API 生成文字描述，与正文一起送入后续分析

**LLM 深读（两步，代码编排 + AI 生成）**：
- **Stage1 推理知识库**：还原作者推理链（研究故事 research_story / 设计决策 design_reasoning / 概念解释 concept_knowledge / 实验推理 experiment_reasoning / 局限），输出结构化 JSON 供报告生成使用
- **一次性生成整篇报告**：参考开源 Paper Explainer 设计，让大模型直接拿论文全文 + 推理知识库连贯输出整篇报告（不分章拼接），prompt 内置防编造铁律——万物有锚（标注 Sec/Table/Fig 出处）、诚实说"未找到"、三类陈述分开（作者主张/客观事实/推断评价）、只写论文里有的

**精读报告结构（Markdown，动态组织，面向科研小白）**：
1. 背景与问题（为什么重要、旧方法为何失败、作者观察到什么）
2. 核心方法（每个设计解决什么问题、为什么这样设计）
3. 关键概念（大白话 + 类比）
4. 实验与证据（区分统计显著与实际意义）
5. 批判性分析（主张-证据映射、消融完整性、baseline 公平性、正文-表格数字一致性）
6. 局限与未来（作者承认的 + 客观存在的）

报告开头含元信息头（标题/作者/会议/年份/日期），内容用大白话类比讲解，公式点到为止。
...
│   ├── researcher.py   # 研究员Agent：并行搜集资料、提炼可追溯证据
│   ├── writer.py       # 写作者Agent：整合生成 Markdown 调研报告
...
│       ├── skill.py            # 实现：解析 + 图表理解 + 推理知识库 + 一次性生成报告
│       └── prompts.py          # LLM 提示词模板（与代码分离，便于单独调优）
...
| 第 3 周 | PDF 解析方案落地（Docling）+ 论文精读 skill | 单篇论文能输出深度精读报告（Markdown、一次性整篇生成，带出处锚点与批判性分析），arXiv 链接可自动转 PDF 解析，图表页通过 API 理解 |
| 第 4 周 | Milvus Lite 图文检索整合 + 行动建议 Agent + 单篇精读历史记录 | 研究员能返回图片并通过 API 理解，行动建议 Agent 能生成 3 个方向建议，单篇精读完成后可存入精读历史、按 PDF/链接搜索历史精读记录 |
| 第 5 周 | 全流程联调 + 数据收集 | 完整闭环跑通，输出 Markdown 调研报告+建议+论文清单，有初步数据 |
...
| 7 | skills/ | 论文精读 skill：单篇论文智能解析 + 推理知识库 + 一次性生成精读报告（带锚点与批判性分析） | 独立能力模块，按 schemas 契约实现 |

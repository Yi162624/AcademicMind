...
[Start]
   │
   ▼
[Planner] ──输出 Outline──► [Verifier_检查大纲]
                               │
                    ┌─────────┴─────────┐
                    ▼                   ▼
               [通过]              [不通过，次数<1]
...
**人类在环（HUMAN_IN_LOOP）**：
当 `outline_retry_count == 1` 或 `report_retry_count == 1` 时，系统**暂停自动流程**，通过 Streamlit 前端向用户展示：
- 当前未通过的具体检查项（如"第2章引用无法验证"）
- 两个按钮：**[接受当前版本并继续]** / **[输入修正意见后重写]**
- 若用户选择接受，流程继续到 Advisor；若用户输入修正意见，将意见写入 `verifier_feedback`，并**重置对应重试计数器**为 0，允许再最多 1 次重试

**状态字段**：
- `outline_retry_count`：大纲重试次数（0-1）
...
```
用户提问 → ① Planner 拆解 → ② 3×Researcher 并行搜集 → ③ Writer 整合生成 → ④ Verifier 质量检查（最多重试 1 次） → ⑤ Advisor 生成建议
```

**第 1 步：规划师拆解问题**
- 确定 3 个子主题："技术原理""临床应用""挑战与趋势"
- 制定研究大纲，标注每个章节需要什么类型图片
- Verifier 检查大纲完整性，不通过则退回 Planner 重拆（最多 1 次）
...
- 将论文分组成若干组（每组一个研究员负责）
- Verifier 检查分析维度与分组是否合理（最多重试 1 次）
...
1. **Verifier 回退 + 人类在环**：非单向流水线，独立 Verifier Agent 质量检查 + 最多 1 次回退重写 + 1 次失败后用户介入，保障学术严谨性

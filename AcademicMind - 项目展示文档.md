...
**人类在环（HUMAN_IN_LOOP）**：系统在两类情况下暂停自动流程，交给用户决定：
1. **大纲确认（必经环节）**：规划师生成大纲后即暂停，通过 Streamlit 前端展示大纲（子主题/章节/图片需求），用户选择 [接受并使用] 或 [输入修改意见]；用户认可后流程才继续，可反复调整
2. **报告复核（兜底）**：报告校验失败且 `report_retry_count == 1` 时暂停，向用户展示未通过的具体检查项（如"第2章引用无法验证"）和两个按钮：**[接受当前版本并继续]** / **[输入修改意见]**；用户输入意见后，将意见写入 `verifier_feedback`，AI 按意见修改后重新校验（最多再 1 次，仍不过则再次进入人工复核）

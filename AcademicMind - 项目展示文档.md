...
2. **报告复核（兜底）**：报告校验失败且 `report_retry_count == 1` 时暂停，向用户展示未通过的具体检查项（如"第2章引用无法验证"）和两个按钮：**[接受当前版本并继续]** / **[输入修正意见后重写]**；若用户输入修正意见，将意见写入 `verifier_feedback`，并**重置报告重试计数器**为 0，允许再最多 1 次重试
